...
const ChemistVisitingAdminById = () => {
  ...
  const fetchUserVisits = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/chemists/visits`);
      if (!response.ok) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => navigate('/admin-dashboard/chemist-visiting')}
            className="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700"
          >
            Back to Chemist Visits
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold text-gray-900">Chemist Visits</h1>
        ...
        {visits.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl shadow-md">
            ...
            <p className="text-gray-600 text-lg">No chemist visits found</p>
            <p className="text-gray-500 mt-2">This user hasn't scheduled any chemist visits yet</p>
          </div>
        ) : (
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                      <div>
                        <span className="font-medium">Contact Person:</span> {visit.chemist?.contactPersonName || 'N/A'}
                      </div>
                      <div>
                        <span className="font-medium">Mobile:</span> {visit.chemist?.mobileNo || 'N/A'}
                      </div>
                      <div>
                        <span className="font-medium">Address:</span> {visit.chemist?.address || 'N/A'}
                      </div>
                      <div>
                        <span className="font-medium">Drug License:</span> {visit.chemist?.drugLicenseNumber || 'N/A'}
                      </div>
                    </div>
                  </div>
                  <div className="ml-4">
...
export default ChemistVisitingAdminById;